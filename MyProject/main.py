...
if __name__ == '__main__':
    ...
    # 初始化模型参数的优化器
    enc_optimizer = torch.optim.SGD(encoder.parameters(), lr=0.001, momentum=0.9)
    dec_optimizer = torch.optim.SGD(decoder.parameters(), lr=0.001, momentum=0.9)
    lstm_optimizer = torch.optim.RMSprop(lstm.parameters(), lr=0.001, momentum=0.9)
    ...
    for epoch in range(0, num_epochs):
        ...
        for i, inputs in enumerate(trainloader):
            ...
            if (i + 1) % 10 == 0:
                print('Epoch:[%d/%d],Step:[%d/%d],Loss:%.4f' % (epoch + 1, num_epochs, i + 1, len(trainset) // batch_size, loss.item()))
