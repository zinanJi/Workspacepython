...
def read():
    BM = pd.read_csv(
        "C:/Jzn/project/DataSet/Legit_csv/ST_KH_JL_BM.csv",
        header=0,
        encoding="ANSI",
        dtype={
            'CLDBS': np.float32,
...
def data_generator(BM, CLD, DA):
    ...
    # BM.pivot(index='CLDBS', columns='SJSJ', values='3-voltage')
    ...
    for i, YHBH in DA['YHBH'].items():
        ...
        data = data.fillna(0)
        print("writting...")
        data.to_csv('F:/data/data' + str(count) + '.csv', encoding="ANSI")
...
BM, CLD, DA = read()
tempData = data_generator(BM, CLD, DA)
